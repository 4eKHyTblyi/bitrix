...
class Product
{
	...
	public function SaveProduct($ID, $IBLOCK_ID, $arProduct, $arPrices, $arStores, $parentID=false)
	{
		...
		$PARENT_IBLOCK_ID = 0;
		if(($arOfferIblock = \CKDAImportUtils::GetOfferIblockByOfferIblock($IBLOCK_ID)) && isset($arOfferIblock['IBLOCK_ID']) && $arOfferIblock['IBLOCK_ID'] > 0) $PARENT_IBLOCK_ID = $arOfferIblock['IBLOCK_ID'];
	...
	public function SaveCatalogSet($ID, $arSet, $setType, $isOffer=false)
	{
		...
		$checkExists = $isEmptySet = $addToExists = false;
		$arItems = array();
		foreach($arSet as $k=>$v)
		{
			$fieldSettings = $this->GetFieldSettings(($isOffer ? 'OFFER_' : '').$fieldPrefix.$k);
			if($k=='ITEM_ID' && $fieldSettings['CHECK_PRODUCTS_EXISTS']=='Y') $checkExists = true;
			if($k=='ITEM_ID' && $fieldSettings['SET_ADD_PRODUCTS_TO_EXISTS']=='Y') $addToExists = true;
			...
			$arVals = array_diff(array_map('trim', explode($sep, $v)), array(''));
		...
		foreach($arItems as $k=>$v)
		{
			...
			elseif(is_array($v['ITEM_ID']))
			{
				$arIds = $v['ITEM_ID'];
				$arItems[$k]['ITEM_ID'] = $v['ITEM_ID'] = array_shift($arIds);
				foreach($arIds as $v2)
				{
					$arItems[] = array_merge($v, array('ITEM_ID'=>$v2));
					if(is_numeric($v2)) $arElementIds[] = $v2;
				}
			}
			if(is_numeric($v['ITEM_ID'])) $arElementIds[] = $v['ITEM_ID'];
			elseif($checkExists) $isEmptySet = true;
		}
		$arCheckedIds = array();
		...
		foreach($arItems as $k=>$v)
		{
			...
			{
				if($checkExists) $isEmptySet = true;
				unset($arItems[$k]);
				continue;
			}
		...
		if(\CCatalogProductSet::isProductHaveSet($ID, $setType))
		{
			...
			if($addToExists)
			{
				...
				{
					$find = false;
					foreach($set['ITEMS'] as $k=>$setItem)
					{
						if($item['ITEM_ID']==$setItem['ITEM_ID'])
						{
							foreach(array('QUANTITY', 'SORT', 'DISCOUNT_PERCENT') as $field)
							{
								if(array_key_exists($field, $item) && $item[$field]!=$setItem[$field]) $set['ITEMS'][$k][$field] = $item[$field];
							}
							$find = true;
						}
					}
					if(!$find) $set['ITEMS'][] = $item;
				}
				$obSet->update($set['SET_ID'], $set);
			}
			else
			{
				if(empty($arItems) || $isEmptySet)
				{
					$obSet->delete($set['SET_ID']);
				}
				else
		...
		elseif(!empty($arItems) && !$isEmptySet)
		{
			$arFields = array(
				'TYPE' => $setType,
				'ITEM_ID' => $ID,
		...
		if($isEmptySet) $this->logger->AddElementChanges($fieldPrefix, array('ERROR'=>'ITEMS_NOT_FOUND'));
		else $this->logger->AddElementChanges($fieldPrefix, $arItems);
	}
	...
	public function SetProductQuantity($ID, $IBLOCK_ID=0)
	{
		...
		if($calcPrice)
		{
			...
			{
				$dbRes = \Bitrix\Catalog\StoreProductTable::GetList(array('filter'=>array('PRODUCT_ID'=>$ID, 'STORE_ID'=>$arStoreKeys), 'select'=>array('STORE_ID', 'AMOUNT')));
				while($arr = $dbRes->Fetch())
				{
					$arFields['ICAT_STORE'.$arr['STORE_ID']] = $this->GetFloatVal($arr['AMOUNT']);
				}
			}
