	...
	<table width="100%" class="kda-ee-field-settings">
		...
		<?if(preg_match('/^ICAT_PRICE\d+_PRICE_DISCOUNT$/', $field) || preg_match('/^ICAT_DISCOUNT_/', $field)){?>
			...
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					<?
					$fName = 'EXTRA'.str_replace('[FIELDS_LIST]', '', $fieldName).'[GROUP_PRICE_ID]';
					$fNameEval = strtr($fName, array("["=>"['", "]"=>"']"));
					eval('$val = $P'.$fNameEval.';');
					?>
		...
		<?if($bPropTypeList){?>
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					<?
					$fName = 'EXTRA'.str_replace('[FIELDS_LIST]', '', $fieldName).'[REL_PROPLIST_FIELD]';
					$fNameEval = strtr($fName, array("["=>"['", "]"=>"']"));
					$val = '';
					if(is_array($PEXTRASETTINGS))
					{
		...
		<tr>
			<td class="kda-ee-settings-margin-container" colspan="2" id="kda-ee-conv-wrap0">
				<?
