...
$moduleImagePath = '/bitrix/panel/'.$moduleId.'/export/images/';
...
if(isset($_FILES) && is_array($_FILES))
{
	...
	foreach($arFileKeys as $fileKey)
	{
		...
		{
			$fid = \CKDAExportUtils::SaveFile($_FILES[$fileKey], $moduleId, false, false, true);
...
if ($REQUEST_METHOD == "POST" && $STEP > 1 && check_bitrix_sessid())
{
	...
	if($ACTION=='SHOW_PREVIEW')
	{
		...
		while((count($arData) < $cntLines) && ($arRes = $ee->GetExportData($list, $cntLines, $i, $si)) && (($arRes['SECTION_COUNT'] > 1 && $arRes['SECTION_COUNT'] >= $arRes['SECTION_KEY'] && ($si == $arRes['SECTION_KEY'] || (($i = 0) || 1)) && ($si = $arRes['SECTION_KEY'])) || /*$i < 2*/ $i <= min(30, max(1, $arRes['PAGE_COUNT']))))
		{
			if(!empty($arRes['FIELDS'])) $arFields = $arRes['FIELDS'];
			if(!empty($arRes['DATA'])) $arData = array_merge($arData, $arRes['DATA']);
			$fullRecCnt = $arRes['FULL_RECORD_COUNT'];
			$i++;
		}
		...
		$sortOfferVal = ($SETTINGS['SORT_OFFER'][$list] ? $SETTINGS['SORT_OFFER'][$list] : $sortVal);
		list($sortBy, $sortOrder) = explode('=>', $sortVal);
		list($sortByOffer, $sortOrderOffer) = explode('=>', $sortOfferVal);
		...
		<div class="kda-ee-title">
			...
			<?//if($fullRecCnt){echo '<span class="kda-ee-full_record-count">'.$fullRecCnt.'</span>';}?>
			<?if($list > 0){?>
		...
		foreach($arFields as $k=>$field)
		{
			...
			if($isSortable)
			{
				$curSortBy = (strpos($field, 'OFFER_')===0 ? $sortByOffer : $sortBy);
				$curSortOrder = (strpos($field, 'OFFER_')===0 ? $sortOrderOffer : $sortOrder);
				$sortClass = 'sort_up';
				$sortTitle = GetMessage("KDA_EE_SETTINGS_SORT_ASC");
				if($curSortBy==$field)
				{
					if($curSortOrder!='DESC')
					{
						$sortClass = 'sort_down';
			...
			echo '<th class="kda-ee-field-select">'.
					'<b>'.CKDAExportUtils::GetColLetterByIndex($k).'</b>'.
					'<div>'.
					...
					'<div>'.
						...
						($isSortable ? '<a href="javascript:void(0);" class="'.$sortClass.'" onclick="EList.Sort(this, \''.(strpos($field, 'OFFER_')===0 ? '_OFFER' : '').'\');" title="'.htmlspecialcharsbx($sortTitle).'"></a>' : '').
					'</div>'.
				'</th>';
		}
		echo '</tr>';
		...
		foreach($arData as $arElement)
		{
			...
			else
			{
				...
				foreach($arFields as $key=>$field)
				{
					...
					if(((isset($fSettings['INSERT_PICTURE']) && $fSettings['INSERT_PICTURE']=='Y') || preg_match('/^(OFFER_)?(IE_QR_CODE_IMAGE|ICAT_BARCODE_IMAGE)$/', $field)) && $ee->IsPictureField($field) && ((is_array($val) && !empty($val)) || (!is_array($val) && strlen($val) > 0)))
					{
						$arVals = $val;
						...
						foreach($arVals as $mkey=>$val)
						{
							...
							if($ee->IsMultipleField($field))
							{
								...
								foreach($arVals2 as $mval)
								{
									...
									{
										$val .= $m[1].'<img src="'.htmlspecialcharsbx($imageDir.$m[2].'?hash='.mt_rand()).'" style="max-width: '.$maxWidth.'px; max-height: '.$maxHeight.'px;">'.$m[3];
									}
									else
									{
										$val .= '<img src="'.htmlspecialcharsbx($imageDir.$mval.'?hash='.mt_rand()).'" style="max-width: '.$maxWidth.'px; max-height: '.$maxHeight.'px;">';
									}
								}
							}
							else
							{
								...
								{
									$val = '<img src="'.htmlspecialcharsbx($imageDir.$val.'?hash='.mt_rand()).'" style="max-width: '.$maxWidth.'px; max-height: '.$maxHeight.'px;">';
								}
							}
							$arVals[$mkey] = $val;
						}
					...
					if(is_array($val) && isset($val['TYPE']) && $val['TYPE']=='MULTICELL')
					{
						...
						foreach($val as $kVal=>$vVal)
						{
							...
							$style = ((is_array($vVal) && isset($vVal['STYLE'])) || isset($arElement['CELLSTYLE_'.$key][$kVal]) ? CKDAExportUtils::GetCellStyleFormatted(array_merge($fSettings, (is_array($vVal) && isset($vVal['STYLE']) ? $vVal['STYLE'] : array())), $SETTINGS_DEFAULT, $arElement['CELLSTYLE_'.$key][$kVal]) : '');
					...
					echo '<td><div class="cell"><div class="cell_inner"'.($SETTINGS_DEFAULT['NOT_SHOW_PREVIEW_STYLES']=='Y' ? '' : ' '.CKDAExportUtils::GetCellStyleFormatted($fSettings, $SETTINGS_DEFAULT, $arElement['CELLSTYLE_'.$key], $arElement['CELLSTYLE_ROW'])).'>'.($isHtml ? nl2br($val) : nl2br(htmlspecialcharsbx($val))).'</div></div></td>';
				}
			}
			echo '</tr>';
		}
...
class_exists('CGlobalCondCtrl');
$obCond = new CKDAExportFilterCondTree();
$boolCond = $obCond->Init(array('FORM_NAME' => 'dataload', 'CONT_ID' => 'tree', 'JS_NAME' => 'JSCatCond'));
if (!$boolCond)
{
	if ($ex = $APPLICATION->GetException())
...
$(document).ready(function(){
	$('a[id$="_add_link"] + select[id$="_add_select"]').unbind('focus').bind('focus', function(){
		var select = $(this);
		var link = select.prev('a');
		if(typeof select.chosen != 'function') return;
		...
		select.one('change', function(){
			...
			BX.fireEvent(select[0], 'change');
		});
		setTimeout(function(){
			var chosenId = select.attr('id')+'_chosen';
			$('#'+chosenId).bind('click', function(e){
				e.stopPropagation();
				return false;
			});
			$(document).one('click', function(e){
				if($('#'+chosenId).length==0) return;
				select.chosen('destroy');
				select.unbind('change');
				BX.fireEvent(select[0], 'blur');
			});
		}, 100);
	});
});
	...
	?>
		...
		<tr>
			<td><?echo GetMessage("KDA_EE_EXPORT_FIELD_CODES"); ?>: <span id="hint_EXPORT_FIELD_CODES"></span><script>BX.hint_replace(BX('hint_EXPORT_FIELD_CODES'), '<?echo GetMessage("KDA_EE_EXPORT_FIELD_CODES_HINT"); ?>');</script></td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("KDA_EE_EXPORT_ELEMENT_ONE_SECTION"); ?>: <span id="hint_EXPORT_ELEMENT_ONE_SECTION"></span><script>BX.hint_replace(BX('hint_EXPORT_ELEMENT_ONE_SECTION'), '<?echo GetMessage("KDA_EE_EXPORT_ELEMENT_ONE_SECTION_HINT"); ?>');</script></td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("KDA_EE_EXPORT_SECTIONS_ONE_CELL"); ?>: <span id="hint_EXPORT_SECTIONS_ONE_CELL_HINT"></span><script>BX.hint_replace(BX('hint_EXPORT_SECTIONS_ONE_CELL_HINT'), '<?echo GetMessage("KDA_EE_EXPORT_SECTIONS_ONE_CELL_HINT"); ?>');</script></td>
			<td>
		...
		<tr <?if($SETTINGS_DEFAULT['EXPORT_SEP_SECTIONS']!='Y'){echo 'style="display: none;"';}?>>
			<td><?echo GetMessage("KDA_EE_EXPORT_SECTION_URL"); ?>:</td>
			<td>
				<input type="checkbox" name="SETTINGS_DEFAULT[EXPORT_SECTION_URL]" value="Y" <?if($SETTINGS_DEFAULT['EXPORT_SECTION_URL']=='Y'){echo 'checked';}?>>
			</td>
		</tr>
		...
		<tr>
			<td><?echo GetMessage("KDA_EE_DISPLAY_LOCK_COLUMNS"); ?>:</td>
			<td>
				<table cellspacing="0"><tr>
				<td style="padding-left: 0px;">
					<input type="checkbox" name="SETTINGS_DEFAULT[DISPLAY_LOCK_COLUMNS]" value="Y" <?if($SETTINGS_DEFAULT['DISPLAY_LOCK_COLUMNS']=='Y'){echo 'checked';}?> onchange="if(this.checked){$('#lock_columns_ext').show();}else{$('#lock_columns_ext').hide();}">
				</td>
				<td>&nbsp; &nbsp; &nbsp;</td>
				<td>
					<div id="lock_columns_ext"<?if($SETTINGS_DEFAULT['DISPLAY_LOCK_COLUMNS']!='Y'){?> style="display: none;"<?}?>>
						<?echo GetMessage("KDA_EE_DISPLAY_LOCK_COLUMNS_CNT");?>:
						<input type="text" name="SETTINGS_DEFAULT[DISPLAY_LOCK_COLUMNS_CNT]" value="<?=htmlspecialcharsbx($SETTINGS_DEFAULT['DISPLAY_LOCK_COLUMNS_CNT'])?>" size="3" placeholder="1">
					</div>
				</td>
				</tr></table>
			</td>
		...
		<tr>
			<td><?echo GetMessage("KDA_EE_EXT_SERVICES_EXPORT_TO_FTP");?>:</td>
			<td>
				<input type="text" name="SETTINGS_DEFAULT[EXPORT_TO_FTP]" value="<?=htmlspecialcharsbx($SETTINGS_DEFAULT['EXPORT_TO_FTP'])?>" placeholder="<?echo GetMessage("KDA_EE_EXT_SERVICES_EXPORT_TO_FTP_EXAMPLE");?>" size="55">
			</td>
		</tr>
			...
			<td>
				...
				<td id="bx24p"<?if($SETTINGS_DEFAULT['EXPORT_TO_BX24']!='Y'){echo ' style="display: none;"';}?>>
					<div id="bx24_help" style="display: none;">
						...
						<?echo sprintf(GetMessage("KDA_EE_EXT_SERVICES_BX24_REST_URL_HELP"), sprintf($imgStr, 'bx24_1.jpg'), sprintf($imgStr, 'bx24_2.jpg'), sprintf($imgStr, 'bx24_3.jpg'), sprintf($imgStr, 'bx24_4.jpg'));?>
					</div>
					<div>
		...
		<tr>
			<td><?echo GetMessage("KDA_EE_EXT_SERVICES_GOOGLE_SPREADSHEETS");?>:</td>
			<td>
				<table cellspacing="0"><tr>
				<td style="padding-left: 0px;"><input type="checkbox" name="SETTINGS_DEFAULT[EXPORT_TO_GOOGLE_SPREADSHEETS]" value="Y" <?=htmlspecialcharsbx($SETTINGS_DEFAULT['EXPORT_TO_GOOGLE_SPREADSHEETS']=='Y' ? 'checked' : '')?> onchange="if(this.checked){$('#googless').show();}else{$('#googless').hide();}"></td>
				<td>&nbsp; &nbsp;</td>
				<td id="googless"<?if($SETTINGS_DEFAULT['EXPORT_TO_GOOGLE_SPREADSHEETS']!='Y'){echo ' style="display: none;"';}?>>
					<div style="margin-bottom: 7px;">
						<?echo GetMessage("KDA_EE_EXT_SERVICES_GOOGLE_TOKEN");?>:
						<div><input type="text" name="SETTINGS_DEFAULT[GOOGLE_TOKEN]" value="<?=htmlspecialcharsbx($SETTINGS_DEFAULT['GOOGLE_TOKEN'])?>" size="50"> &nbsp; <a href="https://accounts.google.com/o/oauth2/auth?client_id=685892932415-87toodq5o9e4vq8pqeh1es86vlcf3oi7.apps.googleusercontent.com&redirect_uri=https://esolutions.su/marketplace/oauth.php&access_type=offline&response_type=code&scope=https://www.googleapis.com/auth/spreadsheets https://www.googleapis.com/auth/userinfo.email" target="_blank"><?echo GetMessage("KDA_EE_EXT_SERVICES_GOOGLE_TOKEN_GET");?></a></div>
					</div>
					<div>
						<?echo GetMessage("KDA_EE_EXT_SERVICES_GOOGLE_SID");?>:
						<div><input type="text" name="SETTINGS_DEFAULT[GOOGLE_SID]" value="<?=htmlspecialcharsbx($SETTINGS_DEFAULT['GOOGLE_SID'])?>" size="50"></div>
					</div>
				</td>
				</tr></table>
			</td>
		...
		<tr>
			<td><?echo GetMessage("KDA_EE_EXT_SERVICES_DELETE_LOCAL_FILE");?>:</td>
			<td>
				<input type="checkbox" name="SETTINGS_DEFAULT[DELETE_LOCAL_FILE]" value="Y" <?=htmlspecialcharsbx($SETTINGS_DEFAULT['DELETE_LOCAL_FILE']=='Y' ? 'checked' : '')?>>
			</td>
		</tr>
		...
		<td colspan="2" id="kda-ee-sheet-list">
			...
			foreach($arKeys as $list)
			{
				...
				<div class="kda-ee-sheet-wrap<?if($ind > 0){echo ' withmargin';}?>">
					...
					?>
					<!--div class="kda-ee-sheet-filter-cnt"><? echo sprintf(GetMessage('KDA_EE_PF_FILTER_COUNT'), (isset($arEtypes[$arTemplate['ETYPE']]) ? $arEtypes[$arTemplate['ETYPE']]['NAME'] : '')); ?>: <b></b></div>-->
					<?$fl = new CKDAEEFieldList($SETTINGS_DEFAULT);?>
					
					
					<div class="kda-ee-sheet-cfilter" id="kda-ee-sheet-cfilter-<?echo $list;?>">
						<div class="kda-ee-sheet-cfilter-hidden">
							<?$fl->ShowSelectFilterFields($iblockId, 'S_FIELD');?>
							<input type="hidden" name="IBLOCK_ID" id="IBLOCK_ID" value="<?echo htmlspecialcharsbx($iblockId);?>">
							<input type="hidden" name="OLD_FILTER" id="OLD_FILTER" value="<?echo (isset($SETTINGS['CFILTER'][$list]) ? htmlspecialcharsbx(CUtil::PhpToJSObject($SETTINGS['CFILTER'][$list])) : '');?>">
						</div>
						<div class="kda-ee-cfilter-field-list"></div>
						<a class="kda-ee-cfilter-add-field" href="javascript:void(0)"><?echo GetMessage('KDA_EE_FILTER_ADD_FIELD');?></a>
					</div>
					<? /new filter */?>
