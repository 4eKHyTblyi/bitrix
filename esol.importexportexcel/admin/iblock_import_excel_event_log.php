...
while($db_res = $rsData->NavNext(true, "a_"))
{
	...
	if($a_TYPE)
	{
		if($a_TYPE=='ELEMENT_NOT_FOUND')
		{
			...
			$a_FIELDS = '<b>'.GetMessage("KDA_IE_EVENTLOG_FILTER_FIELDS").':</b>'.$logger->GetElementDescriptionArray($arFieldsFilter, (bool)($_REQUEST['mode']=='excel'));
			if(!empty($arFieldsFields)) $a_FIELDS .= $logger->GetElementDescription($arFieldsFields, (bool)($_REQUEST['mode']=='excel'));
		}
		elseif(strpos($a_TYPE, 'ELEMENT_')===0 && $a_ENTITY_ID > 0)
		{
			...
			if(strlen($a_FIELDS))
			{
				$a_FIELDS = $logger->GetElementDescription($a_FIELDS, (bool)($_REQUEST['mode']=='excel'));
			}
		}
		elseif(strpos($a_TYPE, 'SECTION_')===0 && $a_ENTITY_ID > 0)
		{
			...
			if(strlen($a_FIELDS))
			{
				$a_FIELDS = $logger->GetSectionDescription($a_FIELDS, $a_IBLOCK_SECTION_IBLOCK_ID, (bool)($_REQUEST['mode']=='excel'));
			}
		}
	}
	...
	$row->AddViewField("ROW_NUMBER", $a_ROW_NUMBER);
