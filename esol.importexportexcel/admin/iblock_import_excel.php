...
if ($REQUEST_METHOD == "POST" && $STEP > 1 && check_bitrix_sessid())
{
	...
	if($ACTION == 'SHOW_REVIEW_LIST')
	{
		...
		?>
			<table class="kda-ie-tbl <?if($bEmptyList){echo 'empty';}?>" data-list-index="<?echo $k;?>" data-iblock-id=<?echo $iblockId;?> data-field-mapping=<?echo $SETTINGS_DEFAULT['FIELD_MAPPING'];?>>
				<tr class="heading">
...
?>
	...
	?>
		...
		<tr>
			...
			<td>
				<input type="checkbox" name="SETTINGS_DEFAULT[ELEMENT_NOT_UPDATE_WO_CHANGES]" value="Y" <?if($SETTINGS_DEFAULT['ELEMENT_NOT_UPDATE_WO_CHANGES']=='Y' || $PROFILE_ID=='new'){echo 'checked';}?>>
			</td>
		</tr>
