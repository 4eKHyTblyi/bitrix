...
<form action="" method="post" enctype="multipart/form-data" name="field_settings">
	...
	<table width="100%" class="kda-ie-price-calculating">
		...
		<tr>
			<td class="adm-detail-content-cell-l"><?echo GetMessage("KDA_IE_CALC_PRICE_EMPTY_ACTION"); ?></td>
			<td class="adm-detail-content-cell-r">
				<select name="DEFAULTS[PRICE_EMPTY_ACTION]">
					<option value="DELETE"<?if($DEFAULTS['PRICE_EMPTY_ACTION']=='DELETE'){echo ' selected';}?>><?echo GetMessage("KDA_IE_CALC_PRICE_EMPTY_ACTION_DELETE");?></option>
					<option value="SALE_OLD"<?if($DEFAULTS['PRICE_EMPTY_ACTION']=='SALE_OLD'){echo ' selected';}?>><?echo GetMessage("KDA_IE_CALC_PRICE_EMPTY_ACTION_SALE_OLD");?></option>
				</select>
			</td>
		</tr>
...
{
	$arStores['ICAT_STORE'.$arStore["ID"]] = GetMessage("KDA_IE_CALC_STORE_PREFIX").' "'.(strlen($arStore["TITLE"]) > 0 ? $arStore["TITLE"] : $arStore["ADDRESS"]).'"';
}
...
{
	$arPriceTypes["ICAT_PRICE".$arPriceType["ID"]] = GetMessage("KDA_IE_CALC_PRICE_PREFIX").' "'.(strlen($arPriceType["NAME_LANG"]) > 0 ? $arPriceType["NAME_LANG"] : $arPriceType["NAME"]).'"';
}
  ...
  <div class="kda-ie-price-calculating-iblock">&nbsp;
	<div style="display: none;">
		<select name="price">
			...
			{
				?><optgroup label="<?echo GetMessage("KDA_IE_CALC_PROPS")?>"><?
			...
			{
				?><optgroup label="<?echo GetMessage("KDA_IE_CALC_PRICES")?>"><?
				foreach($arFieldsPrice as $k=>$v)
				{
					if(!in_array($k, $arGroupFields)) continue;
					?><option value="<?echo $k; ?>"><?echo htmlspecialcharsbx($v); ?></option><?
		...
		<select name="quantity">
			...
			{
				?><optgroup label="<?echo GetMessage("KDA_IE_CALC_STORES")?>"><?
				foreach($arFieldsQnt as $k=>$v)
				{
					if(!in_array($k, $arGroupFields)) continue;
					?><option value="<?echo $k; ?>"><?echo htmlspecialcharsbx($v); ?></option><?
				  ...
				  <tr data-index="<?echo $index;?>">
					<td>
					  <div class="kda-ie-select-mapping">
						...
						<a href="javascript:void(0)" onclick="EProfile.RelTablePriceShowSelect(this, 'quantity', true)" data-default-text="<?echo GetMessage("KDA_IE_NOT_CHOOSE")?>"><?echo (strlen($arMap['quantity']) > 0 && isset($arFieldsQnt[$arMap['quantity']]) ? $arFieldsQnt[$arMap['quantity']] : GetMessage("KDA_IE_NOT_CHOOSE"))?></a>
					  </div>
					</td>
					<td>
					  <div class="kda-ie-select-mapping">
						<input type="hidden" name="MAP[<?echo $iblockId;?>][<?echo $index;?>][price]" value="<?echo htmlspecialcharsbx($arMap['price']);?>">
						<a href="javascript:void(0)" onclick="EProfile.RelTablePriceShowSelect(this, 'price', true)" data-default-text="<?echo GetMessage("KDA_IE_NOT_CHOOSE")?>"><?echo (strlen($arMap['price']) > 0 && isset($arFieldsPrice[$arMap['price']]) ? $arFieldsPrice[$arMap['price']] : GetMessage("KDA_IE_NOT_CHOOSE"))?></a>
					  </div>
					</td>
					<td>
