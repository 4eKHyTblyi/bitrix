...
class CKDAExportExcelHighload {
	...
	public function Export()
	{
		...
		while(!$break)
		{
			...
			if($break)
			{
				$break = ($break && ($listIndex == $lastListIndex));
				if(!$break)
				{
					$lastPage = $page = 1;
					reset($arListIndexes);
	...
	public function GetFieldValue($arProp, $val, $relField=false)
	{
		...
		{
			$val = $this->GetFieldElementValue($arProp, $val, $relField);
		}
	...
	public function GetFieldElementValue($arProp, $val, $relField=false)
	{
		if($val)
		{
			$selectField = 'NAME';
	...
	public function GetCurHlblId()
	{
		$hlblId = $this->params['HIGHLOADBLOCK_ID'];
		$changeHlblId = (bool)(isset($this->params['CHANGE_HLBL_ID'][$this->listIndex]) && $this->params['CHANGE_HLBL_ID'][$this->listIndex]=='Y');
		if($changeHlblId && $this->params['LIST_HLBL_ID'][$this->listIndex])
		{
			$hlblId = $this->params['LIST_HLBL_ID'][$this->listIndex];
		}
		return $hlblId;
	}
	...
	public function GetExportData($listIndex, $limit=10, $page=1)
	{
		...
		$hlblId = $this->GetCurHlblId();
		
		if(!isset($this->filters)) $this->filters = array();
		if(!isset($this->filters[$hlblId])) $this->filters[$hlblId] = array();
		if(!isset($this->filters[$hlblId][$listIndex]))
		{
			$arFilter = array();
			...
			$this->filters[$hlblId][$listIndex] = $arFilter;
		}
		else
		{
			$arFilter = $this->filters[$hlblId][$listIndex];
		}
		...
		foreach($arData as $k=>$arElementData)
		{
			...
			if(is_array($this->fparams[$listIndex]))
			{
				foreach($this->fparams[$listIndex] as $fieldIndex=>$arSettings)
				{
					...
					if(isset($arSettings['REL_ELEMENT_FIELD']))
					{
	...
	public function GetElementOrder($hlblId)
	{
		...
		if(count($arOrder)==0) $arOrder = array('ID'=>'ASC');
	...
	public function GetElementsData(&$arData, $arParams)
	{
		...
		foreach($arAllFields as $fieldName)
		{
			...
			{
				list($fieldKey, $fieldRel) = explode('|', $fieldName, 2);
				if(isset($arFieldsHlbl[$fieldKey])) $arPropsFieldsRels[$fieldKey][] = $fieldRel;
			}
		}

		$dbResElements = $entityDataClass::GetList(array(
			'filter' => $arFilter,
			'order' => $this->GetElementOrder($hlblId),
			...
			foreach($arElement as $k=>$v)
			{
				...
				foreach($arRels as $relField)
				{
					if(is_array($v))
					{
						...
						foreach($v as $k2=>$v2)
						{
							$arVals[] = $this->GetFieldValue($arFieldsHlbl[$k], $v2, $relField);
						}
						$arElement[$k.(strlen($relField) > 0 ? '|'.$relField : '')] = $arVals;
					}
					else
					{
						$arElement[$k.(strlen($relField) > 0 ? '|'.$relField : '')] = $this->GetFieldValue($arFieldsHlbl[$k], $v, $relField);
					}
				}
			}
