...
class CKDAExportExcel {
	...
	public function Export()
	{
		...
		if(strlen($dir) > 1 && file_exists($dir) && is_writable($dir))
		{
			$outputFile = $filePath;
		}
		else
	...
	public function CheckExtServices($outputFile)
	{
		...
		if($this->params['EXPORT_TO_GOOGLE_SPREADSHEETS']=="Y" && $this->params['GOOGLE_TOKEN'] && $this->params['GOOGLE_SID'] && function_exists('json_encode'))
		{
			...
			$accessToken = '';
			$ob = new \Bitrix\Main\Web\HttpClient(array('socketTimeout'=>20, 'disableSslVerification'=>false));
			...
			if($accessToken)
			{
				...
				foreach($this->params['LIST_NAME'] as $listIndex=>$listName)
				{
					...
					$listTitle = $arSheets[$listIndex]['properties']['title'];
					if(file_exists($dataFile) && isset($arSheets[$listIndex]))
					{
						/*Clear old data*/
						$maxRow = max(1, $arSheets[$listIndex]['properties']['gridProperties']['rowCount']);
						$maxCol = max(1, $arSheets[$listIndex]['properties']['gridProperties']['columnCount']);
						$maxColLetter = $maxCol;
						$arLNumbers = array();
						while($maxCol > 26)
						{
	...
	public function GetPropertyListValue($arProp, $val, $relField='')
	{
		if($val)
		{
			if(strlen($relField)==0) $relField = 'VALUE';
			$key = $val.'_'.$relField;
	...
	public function GetHighloadBlockValue($arProp, $val, $relField = '')
	{
		if($val && CModule::IncludeModule('highloadblock') && $arProp['USER_TYPE_SETTINGS']['TABLE_NAME'])
		{
			...
			if(!isset($this->propVals[$arProp['ID']][$relField][$val]))
			{
				...
				if(isset($this->hlblFields[$arProp['ID']]) && in_array($relField, $this->hlblFields[$arProp['ID']]))
				{
	...
	public function GetIblockDomain($IBLOCK_ID)
	{
		...
		if(!$this->arIblockDomains[$IBLOCK_ID])
		{
			...
			while($arIBlockSite = $rsIBlockSites->Fetch())
			{
				if($arIBlockSite['SERVER_NAME'] && (!$host || $arIBlockSite['SERVER_NAME']==$_SERVER['HTTP_HOST']))
				{
	...
	public function ConversionReplaceValues($m)
	{
		...
		if(1 || isset($this->currentItemValues[$k]))
		{
				...
				&& preg_match('/^(OFFER_)?(PURCHASING_PRICE|ICAT_PRICE\d+_PRICE(_DISCOUNT)?)$/', $k))
			{
				...
				$value = $this->GetConvertedPrice($value, $this->currentItemValues[$currKey], $this->currentFieldName);
			}
		}
	...
	public function ApplyConversions($val, $arConv, $arItem, $field=false, $iblockFields=array())
	{
		...
		if(is_array($arConv))
		{
			...
			foreach($arConv as $k=>$v)
			{
				...
				{
					$condVal = $this->GetValueForConversion(array_key_exists($v['CELL'], $arItem) ? $arItem[$v['CELL']] : '');
				}
				...
				if(($v['CELL']=='ELSE' && !$execConv)
					...
					|| ($v['WHEN']=='REGEXP' && preg_match('/'.ToLower($v['FROM']).'/i'.CKDAExportUtils::getUtfModifier(), ToLower($condVal)))
					|| ($v['WHEN']=='NOT_REGEXP' && !preg_match('/'.ToLower($v['FROM']).'/i'.CKDAExportUtils::getUtfModifier(), ToLower($condVal)))
					...
					|| ($v['WHEN']=='ANY'))
				{
					...
					elseif($v['THEN']=='REPLACE_SUBSTRING_TO' && strlen($v['FROM']) > 0)
					{
						if($v['WHEN']=='REGEXP')
						{
							if(preg_match('/'.$v['FROM'].'/i'.CKDAExportUtils::getUtfModifier(), $val)) $val = preg_replace('/'.$v['FROM'].'/i'.CKDAExportUtils::getUtfModifier(), $v['TO'], $val);
							else $val = preg_replace('/'.ToLower($v['FROM']).'/i'.CKDAExportUtils::getUtfModifier(), $v['TO'], $val);
						}
					...
					elseif($v['THEN']=='MATH_DIVIDE') $val = $this->GetFloatRoundVal($this->GetFloatVal($val) / $this->GetFloatVal($v['TO']));
							...
							foreach($fieldVal as $k2=>$val)
							{
								if($isMulty)
								{
									...
									else
									{
										...
										$arData[$k][$field.'_'.$fieldIndex][$k2] = $arElementData[$field.'_'.$fieldIndex][$k2] = $newVal = $this->ApplyConversions($val, $arSettings['CONVERSION'], $arElementData2, $field);
								...
								$arData[$k][$field.'_'.$fieldIndex][$k2] = $arElementData[$field.'_'.$fieldIndex][$k2] = $newVal = $this->ApplyConversions($val, $arSettings['CONVERSION'], $arElementData, $field);
								if($newVal===false)
								{
		...
		if($arParams['TYPE'] != 'OFFER')
		{
			...
			if(!empty($arOfferFields) && ($iblockOffer = $this->GetCachedOfferIblock($arFilter['IBLOCK_ID'])))
			{
				...
				if(count($arOfferParams['FILTER']) > 1 && !array_key_exists('ID', $arFilter) && 
					(!isset($arFilter['CATALOG_TYPE']) || !is_array($arFilter['CATALOG_TYPE']) || !defined('\Bitrix\Catalog\ProductTable::TYPE_PRODUCT') || !in_array(\Bitrix\Catalog\ProductTable::TYPE_PRODUCT, $arFilter['CATALOG_TYPE'])))
				{
		...
		foreach($arSections as $skey=>$arSection)
		{
			...
			foreach($arElementList as $arElement)
			{
				...
				if(in_array('QR_CODE_IMAGE', $arElementNameFields) && ((int)$this->stepparams['export_started'] > 0 || (int)$this->stepparams['qrcode_qnt'] < 10))
				{
					...
					$arElement2['IE_QR_CODE_IMAGE'] = ($obRequest->isHttps() ? 'https' : 'http').'://'.$obRequest->getHttpHost().$arElement['DETAIL_PAGE_URL'];
						...
						if(true /*in_array($arProp['ID'], $arPropsFields)*/)
						{
							...
							foreach($arRels as $relField)
							{
								...
								else
								{
									...
									elseif($arProp['PROPERTY_TYPE']=='F')
									{
										$val = $this->GetFileValue($val, $fieldKeyOrig);
									}
				...
				if(!empty($arFieldsPrices))
				{
					foreach($arFieldsPrices as $key=>$arPriceSelectField)
					{
						...
						if(in_array('PRICE_DISCOUNT', $arPriceSelectField))
						{
							...
							if(($optimalPrice = $this->GetProductDiscountPrice($arElement['ID'], $key, array(2), $siteId))!==false)
							{
								$arPrice = array(
							...
							if(isset($this->arPricesGroup[$key]) && is_array($this->arPricesGroup[$key]))
							{
								...
								{
									if(($optimalPrice = $this->GetProductDiscountPrice($arElement['ID'], $key, $arGroups, $siteId))!==false)
									{
										$arPrice = array(
											'DISCOUNT_VALUE' => $optimalPrice['PRICE'],
											'CURRENCY' => $optimalPrice['CURRENCY']
	...
	public function GetProductDiscountPrice($productId, $catalogGroupId, $arUserGroups, $siteId)
	{
		...
		while($arSection = $dbResSections->GetNext())
		{
			...
			foreach($arSection as $key=>$val)
			{
				if(in_array($key, $arSelectField))
				{
					...
					$val = $this->GetSectionField($val, $key, $arUserFields);
	...
	public function GetElementOrder($IBLOCK_ID, $isOffer=false)
	{
		...
		$arOrder = array();
		$arSort = array_map('trim', explode('=>', $this->params['SORT'.($isOffer ? '_OFFER' : '')][$listIndex]));
	...
	public function GetParentSections($ID, $IBLOCK_ID)
	{
		...
		while($parentId)
		{
			...
			{
				if($this->params['EXPORT_SECTION_URL']=='Y' && ($host = $this->GetIblockDomain($IBLOCK_ID))) $arSection['SECTION_PAGE_URL'] = 'http://'.$host.$arSection['SECTION_PAGE_URL'];
	...
	public function GetElementSectionShare(&$arElement2, $arElement, $arElementNameFields, $arFieldsSections, $arFilter=array(), $showOnlyFilterSection=false)
	{
		...
		if(!$showOnlyFilterSection || in_array($baseSectionId, $arFilter['SECTION_ID']) || ($arFilter['INCLUDE_SUBSECTIONS']=='Y' && count(array_intersect($this->GetSectWithParents($baseSectionId), $arFilter['SECTION_ID'])) > 0))
		{
		...
		if($needSectionPath && $arElement['IBLOCK_SECTION_ID'])
		{
			...
			while($arSect = $dbRes->Fetch())
			{
				if($arSect['ID']!=$baseSectionId && (!$showOnlyFilterSection || in_array($arSect['ID'], $arFilter['SECTION_ID']) || ($arFilter['INCLUDE_SUBSECTIONS']=='Y' && count(array_intersect($this->GetSectWithParents($arSect['ID']), $arFilter['SECTION_ID'])) > 0)))
				{
					$arElement['IBLOCK_SECTION_ID'] = $arSect['ID'];
					$this->GetElementSection($arElement3, $arElement, $arElementNameFields, $arFieldsSections);
				}
	...
	public function GetSectionField($val, $key, $arUserFields)
	{
		...
		{
			if($arUserFields[$key]['USER_TYPE_ID']=='enumeration')
			{
