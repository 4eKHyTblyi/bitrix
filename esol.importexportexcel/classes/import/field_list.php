...
class CKDAFieldList {
	...
	public static function GetCatalogDefaultFields($IBLOCK_ID)
	{
		...
		if(CModule::IncludeModule('catalog'))
		{
			...
			if($arCatalog = $dbRes->Fetch())
			{
				...
				$arDefaultCatFields["ICAT_QUANTITY"] = array('NAME' => Loc::getMessage("KDA_IE_FI_QUANTITY"));
				$arDefaultCatFields["ICAT_QUANTITY_RESERVED"] = array('NAME' => Loc::getMessage("KDA_IE_FI_QUANTITY_RESERVED"));
	...
	public function GetIblockSectionFields($i, $IBLOCK_ID = false)
	{
		...
		foreach($arSeoMoreFields as $k=>$v)
		{
			$arSections['ISECT'.$i.'_IPROP_TEMP_SECTION_'.$v['CODE']] = array(
				"name" => Loc::getMessage("KDA_ISECT_FI_SECTION_SEO_FIELD").$v['NAME']
			);
		}
		...
		if(strlen($i)==0)
		{
			...
			$arSections['ISECT'.$i.'_PATH_NAMES'] = array(
				"name" => Loc::getMessage("KDA_ISECT_FI_SECTION_PATH_NAMES")
			);
	...
	public function GetCatalogFieldsCached()
	{
		...
		if(!isset($this->catalogFieldsCached))
		{
			...
			if($arCatalogSetFields = self::GetCatalogSetFields(true))
			{
				foreach($arCatalogSetFields as $v)
				{
					$arCatalogFields[$v['value']] = $v['name'];
				}
			}
			if($arCatalogSetFields = self::GetCatalogSet2Fields(true))
			{
				foreach($arCatalogSetFields as $v)
				{
					$arCatalogFields[$v['value']] = $v['name'];
	...
	public function GetFields($IBLOCK_ID, $offers = false)
	{
		...
		{
			$this->aFields = array();
			$this->aFieldsExData = array();
		}
		
		if(!$this->aFields[$IBLOCK_ID])
		{
			$this->aFieldsExData[$IBLOCK_ID] = array();
			...
			if($arPropFields = self::GetIblockProperties($IBLOCK_ID))
			{
				...
				foreach($arPropFields as $ar)
				{
					...
					$this->aFieldsExData[$IBLOCK_ID][$ar["value"]] = $ar;
				}
			}
	...
	public function GetMultipleFields($IBLOCK_ID)
	{
		$arFields = array();
		...
		foreach($arGroups as $k2=>$v2)
		{
			...
			{
				if(isset($this->aFieldsExData[$IBLOCK_ID][$k]) && $this->aFieldsExData[$IBLOCK_ID][$k]['multiple'])
				{
					$arFields[] = $k;
				}
			}
		...
		foreach($arGroupsOffers as $k2=>$v2)
		{
			foreach($v2['items'] as $k=>$v)
			{
				if(isset($this->aFieldsExData[$arOffer['OFFERS_IBLOCK_ID']][$k]) && $this->aFieldsExData[$arOffer['OFFERS_IBLOCK_ID']][$k]['multiple'])
				{
					$arFields[] = $k;
				}
			}
	...
	public function ShowSelectFields($IBLOCK_ID, $fname, $value="")
	{
		...
		foreach($arGroups as $k2=>$v2)
		{
			...
			foreach($v2['items'] as $k=>$v)
			{
				?><option value="<?echo $k; ?>"<?if($k==$value){echo ' selected';}?><?if(isset($this->aFieldsExData[$IBLOCK_ID][$k])){echo ' data-xmlid="'.htmlspecialcharsbx($this->aFieldsExData[$IBLOCK_ID][$k]['xmlid']).'"';}?>><?echo htmlspecialcharsbx($v); ?></option><?
			}
			?></optgroup><?
		}
		foreach($arGroupsOffers as $k2=>$v2)
		{
			...
			foreach($v2['items'] as $k=>$v)
			{
				?><option value="<?echo $k; ?>"<?if($k==$value){echo ' selected';}?><?if(isset($this->aFieldsExData[$arOffer['OFFERS_IBLOCK_ID']][$k])){echo ' data-xmlid="'.htmlspecialcharsbx($this->aFieldsExData[$arOffer['OFFERS_IBLOCK_ID']][$k]['xmlid']).'"';}?>><?echo htmlspecialcharsbx($v); ?></option><?
			}
			?></optgroup><?
		}
		?></select><?
	...
	public function ShowSelectStoreListForSum($fname, $val=false, $fname2=false, $val2=false)
	{
		...
		if($fname2)
		{
			...
			echo '<select name="'.$fname2.'" onchange="$(\'#quantity_sum_stores_list\').css(\'display\', this.value==\'ALL\' ? \'none\' : \'\')">'.
					'<option value="ALL">'.Loc::getMessage('KDA_IE_PRODUCT_STORES_MODE_ALL').'</option>'.
					'<option value="INC"'.($val2=='INC' ? ' selected' : '').'>'.Loc::getMessage('KDA_IE_PRODUCT_STORES_MODE_INCLUDE').'</option>'.
					'<option value="EXC"'.($val2=='EXC' ? ' selected' : '').'>'.Loc::getMessage('KDA_IE_PRODUCT_STORES_MODE_EXCLUDE').'</option>'.
				'</select>';
		}
	...
	public function GetIblockSettingsFields($IBLOCK_ID, $offers = false)
	{
		...
		if(isset($arGroups['catalog']) && is_array($arGroups['catalog']['items']))
		{
			$arGroupCatalog = array();
			foreach($arGroups['catalog']['items'] as $k=>$v)
			{
				$arGroupCatalog[$k] = $v;
				if(preg_match('/_PRICE$/', $k))
				{
					$arGroupCatalog[$k.'_DISCOUNT'] = $v.' '.Loc::getMessage("KDA_IE_FI_PRICE_WITH_DISCOUNT");
				}
			}
			$arGroups['catalog']['items'] = $arGroupCatalog;
		}
		return $arGroups;
	}
