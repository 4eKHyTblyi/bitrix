...
class CKDAImportUtils {
	...
	public static function MakeFileArray($path, $maxTime = 0)
	{
		...
		$arExt = array('txt', 'csv', 'xls', 'xlsx', 'xlsm', 'dbf');
		if(is_array($path))
		{
			$arFile = $path;
				...
				if(!$bExternalStorage)
				{
					...
					if($download)
					{
						...
						elseif((!$ext || $ext=='php') && $hct && strpos($hct, 'text/html')!==false)
						{
							...
							if(preg_match('/<meta[^>]*http\-equiv=[\'"]?refresh[\'"]?[^>]*>/Uis', $content, $m1) && preg_match('/content=[\'"]\d*\s*;\s*url=([^\'"]*)[\'"]/i', $m1[0], $m2))
							{
							...
							elseif(preg_match('/<h1>Index of/i', $content) && preg_match('/href="([^"]*.('.(implode('|', $arExt)).'))"/Uis', $content, $m1))
							{
								$path = rtrim(preg_replace('/(\?|#).*$/', '', trim($path)), '/').'/'.$m1[1];
								return self::MakeFileArray($path, $maxTime);
							}
							else
							{
		...
		if((in_array($arFile['type'], array('application/zip', 'application/x-zip-compressed', 'application/gzip', 'application/x-gzip', 'application/rar', 'application/x-rar', 'application/x-rar-compressed', 'application/octet-stream')) || in_array($ext, array('zip'))) && !in_array($ext, $arExt))
		{
			...
			else
			{
				...
				if(true /*count(array_diff(scandir($tmpsubdir), array('.', '..')))==0*/)
				{
					...
					if($node1->nodeName=='tr')
					{
						...
						foreach($node1->childNodes as $node2)
						{
							if($node2->nodeName=='td')
							{
								...
								$innerHTML = '"'.$innerHTML.'"';
								if((int)$node2->getAttribute('colspan') > 1)
								{
									$innerHTML = mb_substr(str_repeat(';'.$innerHTML, (int)$node2->getAttribute('colspan')), 1);
								}
								if($col > 0) $innerHTML = ';'.$innerHTML;
								elseif($row > 0) $innerHTML = "\n".$innerHTML;
