...
class CKDAImportExcel {
		...
		if($bElemDeactivate || $bSetDefaultProps || $bSetDefaultProps2)
		{
			...
			while($this->stepparams['deactivate_element_first'] < $this->stepparams['deactivate_element_last'])
			{
				...
				foreach($arFieldsList as $key=>$arFields)
				{
					...
					if($this->stepparams['deactivate_element_first2'][$key] > $this->stepparams['deactivate_element_first'] && $this->stepparams['deactivate_element_first2'][$key] > 0) $arFields['>ID'] = $this->stepparams['deactivate_element_first2'][$key];
		...
		for($column = 0; $column < $this->worksheetColumns; $column++) 
		{
			...
			if($this->params['ELEMENT_LOAD_IMAGES']=='Y')
			{
				if($this->draws[$val->getCoordinate()])
				{
					...
					foreach($this->draws[$val->getCoordinate()] as $draw)
					{
						...
						elseif(strpos($draw, '/')===0 && function_exists('exif_read_data') && ($arExifData = exif_read_data($draw)) && in_array($arExifData['Orientation'], array(3, 6, 8)) && in_array($arExifData['MimeType'], array('image/jpeg', 'image/png')))
						{
							...
							elseif($arExifData['MimeType']=='image/png') imagepng($image, $draw, 9);
							imagedestroy($image);
						}
	...
	public function OnAfterSaveElement($ID)
	{
	...
	public function UpdateElement($ID, $IBLOCK_ID, $arFieldsElement, $arElement=array(), $arElementSections=array(), $isOffer=false)
	{
		...
		{
			$this->PrepareElementPictures($arFieldsElement, $IBLOCK_ID, $isOffer);
	...
	public function PrepareElementPictures(&$arFieldsElement, $IBLOCK_ID, $isOffer=false)
	{
		$iblockFields = $this->GetIblockFields($IBLOCK_ID);
		...
		foreach($arPictures as $picName)
		{
			if($arFieldsElement[$picName])
			{
				...
				$arFile = $this->GetFileArray($val, $arFileParams, 'IE_'.$picName);
	...
	public function SaveSKU($ID, $NAME, $IBLOCK_ID, $arItem, $convertedFields=array())
	{
		...
		while($arElement = $dbRes->Fetch())
		{
			...
			if($this->SaveElementId($OFFER_ID, 'O'))
			{
				...
				if(!$ID && ($eid = $arElement['PROPERTY_'.$OFFERS_PROPERTY_ID.'_VALUE']))
				{
					$arProductIds[] = $eid;
		...
		if($OFFER_ID && ($this->params['ELEMENT_NO_QUANTITY_DEACTIVATE']=='Y' || $this->params['ELEMENT_NO_PRICE_DEACTIVATE']=='Y' || ($this->params['ELEMENT_LOADING_ACTIVATE']=='Y' && !$ID)) && class_exists('\Bitrix\Catalog\ProductTable') && class_exists('\Bitrix\Catalog\PriceTable'))
		{
			foreach($arProductIds as $prodId)
			{
				...
				if(!empty($arOfferIds))
				{
					...
					if($active!==false)
					{
						$dbRes = \Bitrix\KdaImportexcel\DataManager\IblockElementTable::GetListComp(array('ID'=>$prodId, 'CHECK_PERMISSIONS' => 'N'), array('ACTIVE'))->Fetch();
	...
	public function GetFileArray($file, $arParams=array(), $fieldName='')
	{
		$file = $this->Trim($file);
		$arFile = self::GetFileArrayDirect($file, $arParams, $fieldName);
		if(empty($arFile) && strlen($file) > 0 && $file!='-' && strpos($file, $this->params['ELEMENT_MULTIPLE_SEPARATOR'])===false)
		{
	...
	public function GetFileArrayDirect($file, $arParams=array(), $fieldName='')
	{
		...
		$checkFormat = false;
		if($bNeedImage) $fileTypes = array('jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp');
				...
				try{
					...
					while(strlen($loc) > 0 && $loop < 5)
					{
						...
						if(strlen($loc)==0 && strpos($ob->getHeaders()->get('content-type'), 'text/html')!==false && $ob->getStatus()!=404)
						{
							...
							{
								$loc = \Bitrix\KdaImportexcel\IUtils::GetHtmlDomVal(file_get_contents($tempPath2), $fragment, true, (bool)($arParams['MULTIPLE']=='Y'), $file);
		...
		if(is_array($arFile) && array_key_exists('type', $arFile))
		{
			if(strpos($arFile['type'], 'image/')===0)
			{
				...
				/*Tiff convert*/
				if($ext=='tiff' && class_exists('\Imagick'))
				{
					$tmpsubdir = $this->CreateTmpImageDir();
					$file = \Bitrix\Main\IO\Path::convertLogicalToPhysical($tmpsubdir.preg_replace('/\.[^\.]{2,5}\s*$/', '', $arFile['name']).'.jpg');
	...
	public function GetListSection($arFilter, $arSelect, $arOrder=array())
	{
		...
		{
			$dbRes = \CIBlockSection::GetList($arOrder, $arFilter, false, $arKeys);
		}
		return $dbRes;
	}
			...
			foreach($arOldProps as $pk=>$pv)
			{
				...
				if($propsDef[$pk]['MULTIPLE']=='Y')
				{
					...
					if($propsDef[$pk]['PROPERTY_TYPE']!='F' && $saveOldVals)
					{
						...
						if($propsDef[$pk]['PROPERTY_TYPE']=='L') $arProps[$pk] = array_unique($arProps[$pk]);
	...
	public function GetPropValue($arProp, $val)
	{
		...
		{
			$arFile = $this->GetFileArray($val, $arProp, $fieldName);
	...
	public function ApplyConversions($val, $arConv, $arItem, $field=false, $iblockFields=array())
	{
		...
		if(is_array($arConv))
		{
			$execConv = false;
			$this->currentItemValues = $arItem;
			$prefixPattern = '#CELL~*\d+#|#CELL\d+[\-\+]\d+#|#CELL_[A-Z]+\d+#|#CLINK#|#CNOTE#|#HASH#|#FILENAME#|#SHEETNAME#|#IMPORT_PROCESS_ID#|#SEP_SECTION#|#DATETIME#|'.implode('|', $this->rcurrencies);
			...
			foreach($arConv as $k=>$v)
			{
					...
					|| ($v['WHEN']=='ANY'))
				{
					...
					elseif($v['THEN']=='REPLACE_SUBSTRING_TO' && strlen($v['FROM']) > 0)
					{
						if($v['WHEN']=='REGEXP')
						{
							if(preg_match('/'.$v['FROM'].'/i'.CKDAImportUtils::getUtfModifier(), $val)) $val = preg_replace('/'.$v['FROM'].'/i'.CKDAImportUtils::getUtfModifier(), $v['TO'], $val);
							else $val = preg_replace('/'.ToLower($v['FROM']).'/i'.CKDAImportUtils::getUtfModifier(), $v['TO'], $val);
						}
					...
					elseif($v['THEN']=='MATH_ADD_PERCENT') $val = \Bitrix\KdaImportexcel\IUtils::GetFloatRoundVal($this->GetFloatVal($val) * (1 + $this->GetFloatVal($v['TO'])/100));
					elseif($v['THEN']=='MATH_SUBTRACT_PERCENT') $val = \Bitrix\KdaImportexcel\IUtils::GetFloatRoundVal($this->GetFloatVal($val) * (1 - $this->GetFloatVal($v['TO'])/100));
	...
	public function HandleException($exception)
	{
		...
		{
			$error .= (isset($this->phpExpression) ? "<br>".$this->phpExpression."<br>" : "").\Bitrix\Main\Diag\ExceptionHandlerFormatter::format($exception);
		}
		else
		{
