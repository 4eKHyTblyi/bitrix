...
var EList = {
	Init: function()
	{
		...
		$('#kda-ee-sheet-list .kda-ee-sheet').each(function(){
			//new KdaEEFilter($(this).attr('data-sheet-index'));
	...
	UpdateSheet: function(wrap, params)
	{
		...
		var btnsSubmit = $('input[type="submit"][name="submit_btn"], input[type="submit"][name="saveConfigButton"]', $('#dataload'));
		btnsSubmit.prop('disabled', true);
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR)
			{
				...
				if($('#kda-ee-sheet-list .kda-ee-sheet[data-updating="1"]').length==0)
				{
					btnsSubmit.prop('disabled', false);
				}
			},
...
var EProfile = {
	...
	DeleteFromCron: function(btn, key)
	{
		...
		var form = $(btn).closest('form');
		var action = form[0].getAttribute('action');
		$.post(action, 'action=deleterecord&key='+key, function(data){
			obj.UpdateCronRecords(action);
		});
	},
	...
	SendData: function()
	{
		...
		$.ajax({
			...
			error: function(data){
				...
				{
					if(data.responseText.indexOf("[Error]")!=-1 || data.responseText.indexOf("[ErrorException]")!=-1)
					{
						$('#block_error').show();
...
function KdaEEFilter(listIndex, prefix)
{
	...
	{
		var obj = this;
		this.filterBlock = $('#kda-ee-sheet-cfilter-'+this.listIndex);
...
function KdaEEFilterField(filterBlock, fieldPrefix, fieldIndex, filterData, filterKey)
{
	this.Init = function(filterBlock, fieldPrefix, fieldIndex, filterData, filterKey)
	{
		...
		if(this.inGroup)
		{
			$('option[value^="PARENT_"], option[value^="OFFER_"], option[value^="PSECTION_"]'+(this.filterType=='ELEMENT' ? ', option[value^="ISECT_"]' : ''), select).remove();
		}
	...
	this.CreateGroup = function()
	{
		...
		select.bind('change', function(){
			...
			$('.kda-ee-cfilter-field-condlabel', obj.subFilterBlock).html(BX.message("KDA_EE_CONDITION_GROUP_BTN_"+this.value));
	...
	this.GetConditions = function(fname)
	{
		...
		var condKeys = ['EQ', 'NEQ', 'CONTAINS', 'NOT_CONTAINS', 'LT', 'LEQ', 'GT', 'GEQ', 'EMPTY', 'NOT_EMPTY'];
		...
		if(this.fieldType=='DATE') condKeys = ['EQ', 'NEQ', 'LT', 'LEQ', 'GT', 'GEQ', 'EMPTY', 'NOT_EMPTY', 'LAST_N_DAYS', 'NOT_LAST_N_DAYS'];
	...
	this.SetCondEqList = this.SetCondNeqList = function(single)
	{
		var obj = this;
		$.post('esol_massedit_window.php', 'esol_me_action=getfieldvals&field='+this.fieldCode+'&ETYPE='+$('#ETYPE').val()+'&IBLOCK_ID='+$('#IBLOCK_ID').val(), function(data){
			var result = {};
			data = $.trim(data);
			if(data && data.substr(0, 1)=='{' && data.substr(data.length-1)=='}')
			{
			...
			$('div.kda-ee-filter-value-select', obj.valueBlock).remove();
			var div = $('<div class="kda-ee-filter-value-select"></div>');
			div.appendTo(obj.valueBlock);
			var option, select = $('<select name="'+obj.fieldPrefix+'[VALUE][]" multiple></select>');
			if(single) select = $('<select name="'+obj.fieldPrefix+'[VALUE]"></select>');
			select.append('<option value="">'+BX.message("KDA_EE_CHOOSE_VALUE")+'</option>');
			if(result.values)
			{
	...
	this.SetCondEqSection = this.SetCondNeqSection = function(single)
	{
		...
		$.post('esol_massedit_window.php', 'esol_me_action=getfieldvals&field='+this.fieldCode+'&ETYPE='+$('#ETYPE').val()+'&IBLOCK_ID='+$('#IBLOCK_ID').val(), function(data){
			...
			if(obj.filterType=='ELEMENT')
			{
				var chbId = (obj.fieldPrefix+'[INCLUDE_SUBSECTIONS]').replace('/[\[\]]/g', '_');
				$('div.kda-ee-filter-value-chb', obj.valueBlock).remove();
				obj.valueBlock.append('<div class="kda-ee-filter-value-chb"><input type="checkbox" name="'+obj.fieldPrefix+'[INCLUDE_SUBSECTIONS]" value="Y" id="'+chbId+'"><label for="'+chbId+'">'+BX.message("KDA_EE_INCLUDE_SUBSECTIONS")+'</label></div>');
			}
